...
router.post('/register', getValidationMiddleware('register'), async (req, res) => {
  try {
    ...
    // Check if user already exists by email (with case-insensitive search)
    const existingUser = await User.findOne({ 
      email: { $regex: RegExp.create(`^${email.toLowerCase()}$`, 'i') }
    });
    if (existingUser) {
    ...
    // Create user with additional duplicate prevention
    ...
    const lastMinuteCheck = await User.findOne({ 
      email: { $regex: RegExp.create(`^${email.toLowerCase().trim()}$`, 'i') }
    });
    ...
    if (userType === 'service_provider') {
      ...
      const newProvider = ServiceProvider.create({
        user_id: newUser.id,
      ...
      console.log('✅ Service Provider created:', {
        ...
        region: providerLocationData.region,
        district: providerLocationData.district,
        categories: newProvider.service_categories
      });
    }
...
// Add duplicate key error handler middleware
