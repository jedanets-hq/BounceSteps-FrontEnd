...
export const CourseCatalog = () => {
  ...
  const handleManageCourse = (courseId) => {
    if (!managedCourses.includes(courseId)) {
      setManagedCourses([...managedCourses, courseId]);
      setShowManageSuccess(true);
      setTimeout(() => setShowManageSuccess(false), 3000);
    }
  };
        ...
        {filteredCourses.map((course) => (
          <Card key={course.id} className="flex flex-col overflow-hidden transition-all hover:shadow-lg">
            ...
            <CardContent className="space-y-4">
              ...
              {/* Status and Manage */}
              <div className="flex items-center justify-between">
                <Badge variant="default" className="text-sm">{course.status}</Badge>
                <Button 
                  onClick={() => handleManageCourse(course.id)}
                  className={`${managedCourses.includes(course.id) 
                    ? 'bg-green-600 text-white' 
                    : 'bg-gradient-to-r from-primary to-secondary text-white'}`}
                >
                  {managedCourses.includes(course.id) ? 'Managing ✓' : 'Manage Course'}
                </Button>
              </div>
            </CardContent>
          </Card>
